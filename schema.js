class Schema {
  ...
  async fetchSchema() {
    ...
    if (
      grammar.getAttribute('xmlns') !== Schema.XMLNS_RELAXNG ||
...
class SchemaNode extends DevtoolsFormattable {
  ...
  static is(nodeType) {
    ...
    return function nodeIsOfType(node) {
  ...
  dtHeader(cfg) {
    return [
      'div',
      { style: '' },
      ['span', { style: 'font-weight:bold;' }, this.toString()],
  ...
  /** @returns {DTJHTML<this> | null} */
  dtBody(cfg) {
    return ['ol', ...this.treeChildren.map(/** @returns {DTJHTML} */ (n) => ['li', ['object', { object: n }]])];
  }
  ...
  memoizeAttribute(
    property,
    { writable = false, attribute = String(property), allowedValues = /** @type {V[]} */ (undefined) } = {},
  ) {
  ...
  *walkTree() {
  ...
  debugTree(parent, level = 0, childIndex = 0, childLength = 1, prefix = '                ') {
    ...
    const leader = level === 0 ? '' : childIndex === childLength - 1 ? '\\-- ' : '+-- ';
...
class SchemaAlternativeNode extends SchemaNode {
  ...
  get alternatives() {
    ...
    for (const child of this.schemaChildren) {
      if (
        child instanceof SchemaAlternativeNode && (child.tagName === this.tagName || child instanceof SchemaGroupNode)
      ) {
...
class SchemaDefineNode extends SchemaAlternativeNode {
  get name() {
    return this.memoizeAttribute('name');
  }

  get combine() {
    return (this.memoizeAttribute('combine', { writable: true, allowedValues: ['choice', 'interleave', null] }));
  }
  set combine(v) {
    this.memoize('combine', v, true);
  }
  get alternativeType() {
    return this.combine;
  }
  get alternatives() {
    if (
      this.schemaChildren.length === 1 && this.schemaChildren[0] instanceof SchemaAlternativeNode &&
      this.schemaChildren[0].tagName === this.combine
    ) {
...
class SchemaRefNode extends SchemaAlternativeNode {
  ...
  get alternativeType() {
    return this.definition.alternativeType;
  }
  get alternatives() {
    return this.definition.alternatives;
  }
...
class SchemaDataValueNode extends SchemaNode {
  get type() {
    ...
    return this.memoize(
      'type',
      tagName === 'data'
        ? this.xmlElement.getAttribute('type') ?? 'string'
        : tagName === 'value'
        ? this.xmlElement.getAttribute('type') ?? 'token'
        : tagName === 'name'
        ? 'token'
        : tagName,
    );
...
class SchemaRepetitionNode extends SchemaNode {
  get minOccur() {
    return this.memoize('minOccur', this.tagName === 'oneOrMore' ? 1 : 0);
  }
  get maxOccur() {
    return this.memoize('maxOccur', this.tagName === 'optional' ? 1 : Infinity);
  }
}
