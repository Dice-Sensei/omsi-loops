...
class DataElement {
  ...
  bindUI() {
    ...
    for (const [name, required] of Object.entries(this.fieldRequirements ?? {})) {
      for (
        const element
          of /** @type {NodeListOf<HTMLValueElement>} */ (this.uiElement.querySelectorAll(`[name="${name}"]`))
      ) {
        element.addEventListener(
          'input',
          (evt) => this.stringFields[name] = (!element.value?.length && !required) ? undefined : element.value,
        );
      }
    }
    for (
      const evaluationField of this.uiElement.querySelectorAll(
        ':is(.numericEvaluation,.conditionalEvaluation)[data-xml-name]',
      )
    ) {
      ...
      const fieldElement = new (isNumericEvaluation ? NumericEvaluationElement : ConditionalEvaluationElement)(
        this,
        xmlChild ?? tagName,
        isOptional,
      ).spawnUI(evaluationField);
...
class BaseValueElement extends DataElement {
  static {
    ...
    this.addDefaultFields(
      ['tagName'],
      ['actionName', 'skillName', 'buffName', 'resourceName', 'functionName', 'varName'],
    );
  }
...
class EvaluationRulesContainerElement extends DataElement {
  ...
  bindUI() {
    ...
    this.rulesContainer = htmlElement(
      this.uiElement.classList.contains('evaluationRules')
        ? this.uiElement
        : this.uiElement.querySelector('.evaluationRules'),
    );
...
class ValueBearingElement extends EvaluationRulesContainerElement {
  ...
  init() {
    super.init();
    if (
      this.xmlElement?.childNodes.length === 1 && this.xmlElement.firstChild.nodeType === Node.TEXT_NODE &&
      this.value == null
    ) {
      ...
      for (const test of Object.keys(numericTestTypes)) {
        if (this.xmlElement.hasAttribute(test)) {
          if (this.numericTest1 === '') {
            ...
            throw new Error(
              `Too many numeric tests: ${this.numericTest1}, ${this.numericTest2}, ${test} (this is a limitation of the editor, not the spec)`,
            );
          }
        }
      }
...
class DataListBinding {
  ...
  constructor(
    listId,
    optionsSource,
    labelFunc = DataListBinding.identity,
    keyFunc = DataListBinding.identity,
    classFunc,
  ) {
  ...
  init() {
    ...
    if (!this.windowBound) {
      ...
      this.bindDataList(
        'resources',
        resources,
        (_, k) => _txt(`tracked_resources>resource[id=${k}]>label`),
        (k) => k,
        (v) => typeof v,
      );
