...
const reComments = /\/\*[^]*?\*\//g;
const reNestedRule =
  /(?:(?<=[{};])|^)(?<outerSelector>\s*[^{};\s][^{};]*){(?<outerEarly>(?:[^;{}]*;)*(\s*\n)?)(?<innerSelector>[^{};&]*?&[^{};]*){(?<innerRule>[^{}]*)}/g;
...
class CSSNestingPolyfill extends Polyfill {
  ...
  async processRemoteSheet(link) {
    if (globalThis.origin && !link.href.startsWith(globalThis.origin)) return link.sheet;
  ...
  nestSelectors(outerSelector, outerEarly, innerSelector, innerRule) {
    ...
    for (const outer of outerVariants) {
      for (const inner of innerVariants) {
        variants.push(
          inner.includes('&') ? inner.trim().replace('&', outer.trim()) : `${outer.trim()} ${inner.trim()}`,
        );
      }
    }
    const segments = [];
