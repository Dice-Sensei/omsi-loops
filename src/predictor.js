  ...
  Predictor: class {
      ...
      if (data.type === 'error') {
        ...
        if (requestedSnapshots.length === snapshotIds.length) {
          ...
          this.worker.postMessage({
            ...
            snapshotExports: requestedSnapshots.map((s) => s.export()),
          });
        } else {
          console.debug(
            `Only found ${requestedSnapshots.length} of ${snapshotIds.length} requested snapshots. Ignoring request, probably stale.`,
            snapshotIds,
            requestedSnapshots,
          );
        }
    ...
    initElements() {
      ...
      for (const [key, stat] of Object.entries(Koviko.trackedStats)) {
        $('#predictorTrackedStatInput').append(
          '<option value=' + key + " hidden=''>(" + stat.type + ') ' + stat.display_name + '</option>',
        );
      }
    ...
    initPredictions() {
      ...
      this.helpers = this.helpers || {
        ...
        getArmorLevel: (
          r,
          k,
        ) => (1 + (((r.armor || 0) + 3 * (r.enchantments || 0)) * (r.crafts ? h.getGuildRankBonus(r.crafts) : 1)) / 5),
      ...
      const assassinBase = {
        ...
        loop: {
          ...
          tick: (p, a, s, k, r) => (offset) => {
            ...
            let baseSkill = Math.sqrt(getSkillLevelFromExp(k.practical)) + getSkillLevelFromExp(k.thievery) +
          ...
          effect: {
            loop: (r) => r.heart++,
            end: (r, k) => (r.rep += Math.min((r.town + 1) * -250 + getSkillLevelFromExp(k.assassin), 0)),
          },
        },
      };

      const surveyBase = {
        affected: ['map', 'completedMap'],
        canStart: (input) => (input.map > 0),
        effect: (r) => (r.map--, r.completedMap++),
      };
      ...
      const predictions = {
        ...
        'Heal The Sick': {
          ...
          loop: {
            ...
            tick: (p, a, s, k) => (offset) =>
              getSkillLevelFromExp(k.magic) * Math.max(getSkillLevelFromExp(k.restoration) / 50, 1) *
        ...
        'Fight Monsters': {
          ...
          loop: {
            cost: (p, a) => (segment) =>
              fibonacci(Math.floor((p.completed + segment) - p.completed / a.segments + .0000001)) * 10000,
            tick: (p, a, s, k, r) => (offset) =>
        ...
        'Small Dungeon': {
          ...
          loop: {
            ...
            cost: (p, a) => (segment) =>
              precision3(Math.pow(2, Math.floor((p.completed + segment) / a.segments + .0000001)) * 15000),
            tick: (p, a, s, k, r) => (offset) => {
        ...
        'Buy Supplies': {
          ...
          effect: (
            r,
          ) => (r.gold -= 300 - Math.max((r.supplyDiscount || 0) * 20, 0), r.supplies = (r.supplies || 0) + 1),
        },
        'Haggle': {
          affected: ['rep'],
        ...
        'Hitch Ride': { affected: [''], canStart: true, effect: (r, k) => (r.town = 2) },
        'Open Rift': { affected: [''], effect: (r, k) => (r.supplies = 0, r.town = 5, k.dark += 1000) },
        ...
        'Practical Magic': { affected: [''], effect: (r, k) => k.practical += 100 },
        'Learn Alchemy': {
          affected: ['herbs'],
        ...
        'Bird Watching': { affected: [''], canStart: (input) => !!input.glasses },
        ...
        'Continue On': { affected: [''], effect: (r) => r.town = 2 },
        ...
        'Adventure Guild': {
          ...
          loop: {
            ...
            tick: (p, a, s, k, r) => (offset) =>
              (h.getSelfCombat(r, k) + getSkillLevelFromExp(k.magic) / 2) * h.getStatProgress(p, a, s, offset) *
        ...
        'Large Dungeon': {
          ...
          loop: {
            max: (a) => dungeons[a.dungeonNum].length,
            cost: (p, a) => (segment) =>
              precision3(Math.pow(3, Math.floor((p.completed + segment) / a.segments + .0000001)) * 5e5),
            tick: (p, a, s, k, r) => (offset) => {
              let floor = Math.floor(p.completed / a.segments + .0000001);
              return floor in dungeons[a.dungeonNum]
        ...
        'Apprentice': {
          ...
          effect: (r, k) =>
            Math.min(
              (r.apprentice = (r.apprentice || towns[2].expApprentice) + 30 * h.getGuildRankBonus(r.crafts || 0),
                505000),
        ...
        'Mason': {
          ...
          effect: (
            r,
            k,
          ) => (r.mason = Math.min((r.mason || towns[2].expMason) + 20 * h.getGuildRankBonus(r.crafts || 0), 505000),
        ...
        'Architect': {
          ...
          effect: (r, k) =>
            Math.min(
              (r.architect = (r.architect || towns[2].expArchitect) + 10 * h.getGuildRankBonus(r.crafts || 0), 505000),
              k.crafting += 40 * (1 + h.getTownLevelFromExp(r.architect) / 100),
            ),
        },
        'Read Books': { affected: [''], canStart: (input) => input.glasses },
        'Buy Pickaxe': {
          affected: ['gold'],
        ...
        'Start Trek': { affected: [''], effect: (r) => r.town = 3 },
        'Underworld': {
          affected: ['gold'],
        ...
        'Chronomancy': { affected: [''], effect: (r, k) => k.chronomancy += 100 },
        'Looping Potion': {
          affected: ['herbs', 'lpotions'],
          canStart: (input) => (input.herbs >= 400),
          effect: (r, k) => (r.herbs -= 400, r.lpotions = 1, k.alchemy += 100),
        },
        'Pyromancy': { affected: [''], effect: (r, k) => k.pyromancy += 100 * (1 + getBuffLevel('Heroism') * 0.02) },
        ...
        'Hunt Trolls': {
          ...
          loop: {
            cost: (p, a) => (segment) =>
              precision3(Math.pow(2, Math.floor((p.completed + segment) / a.segments + .0000001)) * 1e6),
        ...
        'Face Judgement': { affected: [''], effect: (r) => (r.town = r.rep >= 50 ? 4 : (r.rep <= -50 ? 5 : 3)) },
        'Guru': {
          affected: ['herbs'],
        ...
        'Tidy Up': {
          ...
          loop: {
            ...
            tick: (p, a, s, k) => (offset) =>
              getSkillLevelFromExp(k.practical) * h.getStatProgress(p, a, s, offset) * Math.sqrt(1 + p.total / 100),
            effect: {
              loop: (r) => {
                r.gold += 5;
                r.rep += 1;
        ...
        'Wizard College': {
          ...
          loop: {
            ...
            tick: (p, a, s, k) => (offset) =>
              (getSkillLevelFromExp(k.magic) + getSkillLevelFromExp(k.practical) + getSkillLevelFromExp(k.dark) +
                getSkillLevelFromExp(k.chronomancy) + getSkillLevelFromExp(k.pyromancy) +
                getSkillLevelFromExp(k.restoration) + getSkillLevelFromExp(k.spatiomancy)) *
              h.getStatProgress(p, a, s, offset) * Math.sqrt(1 + p.total / 1000),
            effect: {
              end: (r) => {
                r.gold -= 500;
        ...
        'Spatiomancy': {
          affected: [''],
          manaCost: (r, k) => (20000 / h.getWizardRankBonus(r)),
        ...
        'Build Housing': {
          affected: [''],
          canStart: (input) => {
            return ((input.guild == 'crafting') &&
              ((input.houses || 0) <
                Math.floor(
                  h.getGuildRankBonus(input.crafts || 0) *
                    (1 + Math.min(getSkillLevelFromExp(skills.Spatiomancy.exp), 500) * .01),
                )));
          },
        ...
        'The Spire': {
          ...
          loop: {
            max: (a) => dungeons[a.dungeonNum].length,
            cost: (p, a) => (segment) =>
              precision3(Math.pow(2, Math.floor((p.completed + segment) / a.segments + .0000001)) * 10000000),
            tick: (p, a, s, k, r) => (offset) => {
              const floor = Math.floor(p.completed / a.segments + .0000001);
        ...
        'Rescue Survivors': {
          ...
          loop: {
            cost: (p, a) => (segment) =>
              fibonacci(2 + Math.floor((p.completed + segment) / a.segments + .0000001)) * 5000,
            tick: (p, a, s, k) => (offset) =>
              getSkillLevelFromExp(k.magic) * Math.max(getSkillLevelFromExp(k.restoration) / 100, 1) *
              h.getStatProgress(p, a, s, offset) * Math.sqrt(1 + p.total / 100),
            effect: {
              end: (r, k) => {
        ...
        'Thieves Guild': {
          ...
          loop: {
            ...
            effect: {
              ...
              segment: (r, k) => (r.gold += 10, r.thieves = (r.thieves || 0) + 1, k.practical += 50, k.thievery += 50),
            },
          },
        },
        'Pick Pockets': {
    ...
    async update(actions, container, isDebug) {
        ...
        state = {
          ...
          stats: statList.reduce(
            (
              stats,
              name,
            ) => (stats[name] = getExpOfLevel(buffs.Imbuement2.amt * (skills.Wunderkind.exp >= 100 ? 2 : 1)), stats),
            {},
          ),
      ...
      const snapshots = {
        ...
        currProgress: new Koviko.Snapshot({
          ...
          'Hunt Trolls': 0,
          'Tidy Up': 0,
          'Fight Frost Giants': 0,
          'The Spire': 0,
          'Fight Jungle Monsters': 0,
          'Rescue Survivors': 0,
          'Heroes Trial': 0,
          'Dead Trial': 0,
          'Secret Trial': 0,
        ...
        toNextLoop: new Koviko.Snapshot({
          'Fight Monsters': 0,
          'Heal The Sick': 0,
          'Small Dungeon': 0,
          'Large Dungeon': 0,
      ...
      const affected = Object.keys(
        actions.reduce((stats, x) =>
          (x.name in this.predictions && this.predictions[x.name].affected || []).reduce(
            (stats, name) => (stats[name] = true, stats),
            stats,
          ), {}),
      );
      ...
      for await (
        ...
        // Update the view
        if (div) {
          div.querySelector('ul.koviko').outerHTML = this.template(
            listedAction.name,
            affected,
            state.resources,
            snapshots,
      ...
      if (this.update.id === id) {
        ...
        const updateTime = performance.measure(
          'predictor-totalupdate',
          'start-predictor-update',
    ...
    async *doUpdate(runData) {
        ...
        ? class WorkerRunInfo {
          async start() {
            ...
            const initdelay = performance.measure(
              'predictor-worker-initdelay',
              'enter-predictor-worker',
              'start-predictor-worker',
            );
      ...
      for (const [i, listedAction] of actions.entries()) {
        ...
        let cache = cacheIsValid && i !== finalIndex
          ? this.cache.next([listedAction.name, listedAction.loops, listedAction.disabled])
          : null;
        ...
        if (prediction) {
          let repeatLoop = options.predictorRepeatPrediction && options.repeatLastAction && (i == finalIndex) &&
            (prediction.action.allowed == undefined);
          ...
          if (!cache || i == finalIndex) {
            ...
            if (isValid) {
              for (loop; repeatLoop ? isValid : loop < listedAction.loops; loop++) {
                let canStart = typeof (prediction.canStart) === 'function'
                  ? prediction.canStart(state.resources)
                  : prediction.canStart;
                ...
                if ((i == finalIndex) && (prediction.ticks() == 1) && (!prediction.loop) && (loop > 0)) {
                  ...
                  prediction.loop && prediction.loop.max &&
                  ((prediction.loop.max(prediction.action) * prediction.action.segments) <=
                    state.progress[prediction.name].completed)
                ) {
                  break;
                } else {
                  // Run the prediction
            ...
            if (prediction.name in state.progress) {
              state.currProgress[prediction.name] = state.progress[prediction.name].completed /
                prediction.action.segments;
              state.toNextLoop[prediction.name] = state.progress[prediction.name].progress != 0
                ? state.progress[prediction.name].progress / state.progress[prediction.name].loopTotalCost
                : 0;
            }
            // Update the cache
            if (i !== finalIndex) {
    ...
    async finishUpdate(container, runData) {
      ...
      switch (trackedStat.type) {
        case 'R':
          if (trackedStat.name == 'soul') {
            ...
            newStatisticValue = getExploreSkill() * (state.resources.completedMap + 3 * state.resources.submittedMap) /
              totalMinutes;
            legend = 'Survey';
      ...
      container &&
        (this.totalDisplay.innerHTML = intToString(total) + ' | ' + this.timeString(state.resources.totalTicks) +
          ' | ');
      switch (trackedStat.type) {
        case 'TIME':
    ...
    template(currname, affected, resources, snapshots, isValid) {
      ...
      //Timer
      tooltip += '<tr><td><b>TIME</b></td><td>' + precision3(resources.totalTicks / 50) + '</td><td>(+' +
      ...
      return `<ul class='koviko ${isValidStr}'>` + Affec +
        `<div class='koviko showthis'><table>${tooltip || '<b>N/A</b>'}</table></div></ul>`;
    }
    ...
    tick(prediction, state) {
      ...
      // @ts-ignore
      if (prediction.action.expMult != 0) {
        prediction.exp(prediction.action, state.stats, state.talents, state.soulstones);
      }
      ...
      if (prediction.loop) {
         ...
         * @type {number}
         */
        let additionalProgress = tickProgress(segment) *
          (prediction.baseManaCost(prediction.action) / prediction.ticks());
