import { Buff, LevelExp, Skill, Stat } from './stats.ts';
...
export class Data {
  ...
  static getSnapshotIndex(identifier) {
    return identifier instanceof DataSnapshot
      ...
      ? this.findSnapshotIndex((s) => s.tag === identifier)
      : typeof identifier === 'string'
      ? this.findSnapshotIndex((s) => s.name.split(':', 1)[0] === identifier)
      : typeof identifier === 'number'
      ? (identifier >= 0 ? identifier : this.snapshotStack.length + identifier)
      : typeof identifier?.id === 'number'
      ? this.findSnapshotIndex((s) => s.id === identifier.id)
      : -1;
  }
  ...
  static importSnapshot(data, baseSnapshot) {
    ...
    const snapshot = data.type === 'static'
      ? new StaticSnapshot(data.name, data.id)
      : data.type === 'delta'
      ? new DeltaSnapshot(data.name, baseSnapshot, data.id)
      : null;
  ...
  static rewindToSnapshot(identifier, keepOnStack) {
    ...
    let inverse = new DeltaSnapshot('current', this.snapshotStack[this.snapshotStack.length - 1]).recordData(
      this.rootObjects,
    )?.invert();
    let snap;
