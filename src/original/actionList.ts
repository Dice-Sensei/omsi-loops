...
class AssassinAction extends MultipartAction {
  ...
  tickProgress(
    _offset,
    _loopCounter,
    totalCompletions = towns[this.townNum]['total' + this.varName],
  ) {
    let baseSkill = Math.sqrt(getSkillLevel('Practical')) +
      getSkillLevel('Thievery') + getSkillLevel('Assassin');
    let loopStat = 1 / 10;
  ...
  return ({
    ...
    finish() {
      if (towns[this.townNum].getLevel('Survey') != 100) {
        viewalThis.driver.addResource('map', -1);
...
export function adjustLocks() {
  ...
  town.totalLocks = Math.floor(
    baseLocks * getSkillMod('Spatiomancy', 100, 300, .5) +
      baseLocks * getSurveyBonus(town),
  );
}
...
Action.PickLocks = new Action('Pick Locks', {
  ...
  goldCost() {
    let base = 10;
    return Math.floor(
      base * getSkillMod('Practical', 0, 200, 1) * getSkillBonus('Thievery'),
    );
  },
  finish() {
    towns[0].finishRegular(this.varName, 10, () => {
...
Action.BuyManaZ1 = new Action('Buy Mana Z1', {
  ...
  goldCost() {
    return Math.floor(
      50 * getSkillBonus('Mercantilism') * adjustGoldCostFromPrestige(),
    );
  },
  finish() {
...
export function adjustSQuests() {
  ...
  town.totalSQuests = Math.floor(
    baseSQuests * getSkillMod('Spatiomancy', 200, 400, .5) +
      baseSQuests * getSurveyBonus(town),
  );
}
...
Action.TrainStrength = new Action('Train Strength', {
  ...
  storyReqs(storyNum) {
    switch (storyNum) {
      ...
      case 2:
        return storyFlags.strengthTrained && getTalent('Str') >= 100;
      case 3:
        return storyFlags.strengthTrained && getTalent('Str') >= 1000;
      case 4:
        return storyFlags.strengthTrained && getTalent('Str') >= 10000;
      case 5:
        return storyFlags.strengthTrained && getTalent('Str') >= 100000;
    }
    return false;
  },
  stats: {
...
export function adjustLQuests() {
  ...
  town.totalLQuests = Math.floor(
    baseLQuests * getSkillMod('Spatiomancy', 300, 500, .5) +
      baseLQuests * getSurveyBonus(town),
  );
}
...
Action.HealTheSick = new MultipartAction('Heal The Sick', {
  ...
  story(completed) {
    if (towns[0].HealLoopCounter / 3 + 1 >= 10) {
      setStoryFlag('heal10PatientsInALoop');
    }
  },
});
...
Action.SmallDungeon = new DungeonAction('Small Dungeon', 0, {
  ...
  story(completed) {
    ...
    if (towns[this.townNum][this.varName + 'LoopCounter'] >= 42) {
      setStoryFlag('clearSDungeon');
    }
  },
});
...
export function adjustHunt() {
  ...
  town.totalHunt = Math.floor(
    baseHunt * getSkillMod('Spatiomancy', 400, 600, .5) +
      baseHunt * getSurveyBonus(town),
  );
}
export function adjustHerbs() {
  ...
  town.totalHerbs = Math.floor(
    baseHerbs * getSkillMod('Spatiomancy', 500, 700, .5) +
      baseHerbs * getSurveyBonus(town),
  );
}
...
Action.SitByWaterfall = new Action('Sit By Waterfall', {
  ...
  storyReqs(storyNum) {
    switch (storyNum) {
      ...
      case 2:
        return storyFlags.satByWaterfall && getTalent('Soul') >= 100;
      case 3:
        return storyFlags.satByWaterfall && getTalent('Soul') >= 1000;
      case 4:
        return storyFlags.satByWaterfall && getTalent('Soul') >= 10000;
      case 5:
        return storyFlags.satByWaterfall && getTalent('Soul') >= 100000;
    }
    return false;
  },
  stats: {
...
Action.TalkToHermit = new Action('Talk To Hermit', {
  ...
  finish() {
    towns[1].finishProgress(
      this.varName,
      50 * (1 + towns[1].getLevel('Shortcut') / 100),
    );
...
Action.TrainDexterity = new Action('Train Dexterity', {
  ...
  storyReqs(storyNum) {
    switch (storyNum) {
      ...
      case 2:
        return storyFlags.dexterityTrained && getTalent('Dex') >= 100;
      case 3:
        return storyFlags.dexterityTrained && getTalent('Dex') >= 1000;
      case 4:
        return storyFlags.dexterityTrained && getTalent('Dex') >= 10000;
      case 5:
        return storyFlags.dexterityTrained && getTalent('Dex') >= 100000;
    }
    return false;
  },
  stats: {
...
Action.TrainSpeed = new Action('Train Speed', {
  ...
  storyReqs(storyNum) {
    switch (storyNum) {
      ...
      case 2:
        return storyFlags.speedTrained && getTalent('Spd') >= 100;
      case 3:
        return storyFlags.speedTrained && getTalent('Spd') >= 1000;
      case 4:
        return storyFlags.speedTrained && getTalent('Spd') >= 10000;
      case 5:
        return storyFlags.speedTrained && getTalent('Spd') >= 100000;
    }
    return false;
  },
  stats: {
...
Action.BirdWatching = new Action('Bird Watching', {
  ...
  storyReqs(storyNum) {
    switch (storyNum) {
      ...
      case 2:
        return storyFlags.birdsWatched && getTalent('Per') >= 100;
      case 3:
        return storyFlags.birdsWatched && getTalent('Per') >= 1000;
      case 4:
        return storyFlags.birdsWatched && getTalent('Per') >= 10000;
      case 5:
        return storyFlags.birdsWatched && getTalent('Per') >= 100000;
    }
    return false;
  },
  stats: {
...
Action.DarkRitual = new MultipartAction('Dark Ritual', {
  ...
  canStart(loopCounter = towns[this.townNum].DarkRitualLoopCounter) {
    return resources.reputation <= -5 && loopCounter === 0 && checkSoulstoneSac(this.goldCost()) &&
      getBuffLevel('Ritual') < getBuffCap('Ritual');
  },
  loopCost(segment) {
...
export function adjustSuckers() {
  ...
  town.totalGamble = Math.floor(
    baseGamble * getSkillMod('Spatiomancy', 600, 800, .5) +
      baseGamble * getSurveyBonus(town),
  );
}
...
Action.SellPotions = new Action('Sell Potions', {
  ...
  finish() {
    ...
    addResource(
      'gold',
      resources.potions * getSkillLevel('Alchemy'),
    );
...
Action.AdventureGuild = new MultipartAction('Adventure Guild', {
  ...
  loopsFinished() {
    if (vals.curAdvGuildSegment >= 0) setStoryFlag('advGuildRankEReached');
    if (vals.curAdvGuildSegment >= 3) setStoryFlag('advGuildRankDReached');
    if (vals.curAdvGuildSegment >= 6) setStoryFlag('advGuildRankCReached');
    if (vals.curAdvGuildSegment >= 9) setStoryFlag('advGuildRankBReached');
    if (vals.curAdvGuildSegment >= 12) setStoryFlag('advGuildRankAReached');
    if (vals.curAdvGuildSegment >= 15) setStoryFlag('advGuildRankSReached');
    if (vals.curAdvGuildSegment >= 27) setStoryFlag('advGuildRankUReached');
    if (vals.curAdvGuildSegment >= 39) setStoryFlag('advGuildRankGodlikeReached');
  },
  segmentFinished() {
...
Action.CraftingGuild = new MultipartAction('Crafting Guild', {
  ...
  loopsFinished() {
    if (vals.curCraftGuildSegment >= 0) setStoryFlag('craftGuildRankEReached');
    if (vals.curCraftGuildSegment >= 3) setStoryFlag('craftGuildRankDReached');
    if (vals.curCraftGuildSegment >= 6) setStoryFlag('craftGuildRankCReached');
    if (vals.curCraftGuildSegment >= 9) setStoryFlag('craftGuildRankBReached');
    if (vals.curCraftGuildSegment >= 12) setStoryFlag('craftGuildRankAReached');
    if (vals.curCraftGuildSegment >= 15) setStoryFlag('craftGuildRankSReached');
    if (vals.curCraftGuildSegment >= 27) setStoryFlag('craftGuildRankUReached');
    if (vals.curCraftGuildSegment >= 39) setStoryFlag('craftGuildRankGodlikeReached');
  },
  segmentFinished() {
...
Action.Mason = new Action('Mason', {
  ...
  unlocked() {
    return towns[2].getLevel('Drunk') >= 60 &&
      towns[2].getLevel('Apprentice') >= 100;
  },
  finish() {
...
Action.ReadBooks = new Action('Read Books', {
  ...
  storyReqs(storyNum) {
    switch (storyNum) {
      ...
      case 2:
        return storyFlags.booksRead && getTalent('Int') >= 100;
      case 3:
        return storyFlags.booksRead && getTalent('Int') >= 1000;
      case 4:
        return storyFlags.booksRead && getTalent('Int') >= 10000;
      case 5:
        return storyFlags.booksRead && getTalent('Int') >= 100000;
    }
    return false;
  },
  stats: {
...
Action.HeroesTrial = new TrialAction('Heroes Trial', 0, {
  ...
  grantsBuff: 'Heroism',
  floorReward() {
    if (this.currentFloor() >= getBuffLevel('Heroism')) {
      addBuffAmt('Heroism', 1, this);
    }
...
export function adjustMineSoulstones() {
  ...
  town.totalMineSoulstones = Math.floor(
    baseMine * getSkillMod('Spatiomancy', 700, 900, .5) +
      baseMine * getSurveyBonus(town),
  );
}
...
export function adjustArtifacts() {
  ...
  town.totalArtifacts = Math.floor(
    baseArtifacts * getSkillMod('Spatiomancy', 800, 1000, .5) +
      baseArtifacts * getSurveyBonus(town),
  );
}
...
Action.ImbueMind = new MultipartAction('Imbue Mind', {
  ...
  storyReqs(storyNum) {
    switch (storyNum) {
      case 1:
        return storyFlags.imbueMindThirdSegmentReached ||
          getBuffLevel('Imbuement') >= 1;
      case 2:
  ...
  canStart(loopCounter = towns[3].ImbueMindLoopCounter) {
    return loopCounter === 0 && checkSoulstoneSac(this.goldCost()) &&
      getBuffLevel('Imbuement') < getBuffCap('Imbuement');
  },
  loopCost(segment) {
...
Action.ImbueBody = new MultipartAction('Imbue Body', {
  ...
  storyReqs(storyNum) {
    switch (storyNum) {
      case 1:
        return storyFlags.imbueBodyThirdSegmentReached ||
  ...
  canStart(loopCounter = towns[3].ImbueBodyLoopCounter) {
    ...
    for (const stat of statList) {
      if (getTalent(stat) < getBuffLevel('Imbuement2') + 1) {
        tempCanStart = false;
      }
    }
    return loopCounter === 0 &&
      (getBuffLevel('Imbuement') >
  ...
  loopsFinished() {
    ...
    for (const stat of statList) {
      ...
      const targetTalentLevel = Math.max(
        currentTalentLevel - getBuffLevel('Imbuement2') - 1,
        0,
      );
...
export function adjustDonations() {
  ...
  town.totalDonations = Math.floor(
    base * getSkillMod('Spatiomancy', 900, 1100, .5) + base * getSurveyBonus(town),
  );
}
...
Action.Mercantilism = new Action('Mercantilism', {
  ...
  storyReqs(storyNum) {
    switch (storyNum) {
      case 1:
        return getSkillLevel('Mercantilism') >= 1;
      case 2:
        return getSkillLevel('Mercantilism') >= 30;
      case 3:
        return getSkillLevel('Mercantilism') >= 100;
      case 4:
        return getSkillLevel('Mercantilism') >= 500;
    }
  },
  stats: {
    Per: 0.2, // Temp
...
Action.CharmSchool = new Action('Charm School', {
  ...
  storyReqs(storyNum) {
    switch (storyNum) {
      ...
      case 2:
        return storyFlags.charmSchoolVisited && getTalent('Cha') >= 100;
      case 3:
        return storyFlags.charmSchoolVisited && getTalent('Cha') >= 1000;
      case 4:
        return storyFlags.charmSchoolVisited && getTalent('Cha') >= 10000;
      case 5:
        return storyFlags.charmSchoolVisited && getTalent('Cha') >= 100000;
    }
  },
  stats: {
    Cha: 0.8,
...
Action.Oracle = new Action('Oracle', {
  ...
  storyReqs(storyNum) {
    switch (storyNum) {
      ...
      case 2:
        return storyFlags.oracleVisited && getTalent('Luck') >= 100;
      case 3:
        return storyFlags.oracleVisited && getTalent('Luck') >= 1000;
      case 4:
        return storyFlags.oracleVisited && getTalent('Luck') >= 10000;
      case 5:
        return storyFlags.oracleVisited && getTalent('Luck') >= 100000;
    }
  },
  stats: {
    Luck: 0.8,
...
Action.WizardCollege = new MultipartAction('Wizard College', {
  ...
  tickProgress(offset, _loopCounter, totalCompletions = towns[4][`total${this.varName}`]) {
    return (
      getSkillLevel('Magic') + getSkillLevel('Practical') +
      getSkillLevel('Dark') +
      getSkillLevel('Chronomancy') + getSkillLevel('Pyromancy') +
      getSkillLevel('Restoration') +
      getSkillLevel('Spatiomancy')
    ) *
      Math.sqrt(1 + totalCompletions / 1000);
  },
  loopsFinished() {
  ...
  segmentFinished() {
    ...
    increaseStoryVarTo('maxWizardGuildSegmentCleared', vals.curWizCollegeSegment);
  },
  getPartName() {
  ...
  storyReqs(storyNum) {
    switch (storyNum) {
      ...
      case 2:
        return getSkillLevel('Restoration') >= 50;
      case 3:
        return getSkillLevel('Restoration') >= 200;
      case 4:
        return getSkillLevel('Restoration') >= 500;
    }
  },
  stats: {
    Int: 0.5,
  ...
  storyReqs(storyNum) {
    switch (storyNum) {
      case 1:
        return getSkillLevel('Spatiomancy') >= 1;
      case 2:
        return getSkillLevel('Spatiomancy') >= 50;
      case 3:
        return getSkillLevel('Spatiomancy') >= 200;
      case 4:
        return getSkillLevel('Spatiomancy') >= 600;
      case 5:
        return getSkillLevel('Spatiomancy') >= 1000;
      case 6:
        return getSkillLevel('Spatiomancy') >= 1500;
    }
  },
  stats: {
    Int: 0.6,
...
Action.CollectTaxes = new Action('Collect Taxes', {
  ...
  finish() {
    const goldGain = Math.floor(
      resources.houses * getSkillLevel('Mercantilism') / 10,
    );
    addResource('gold', goldGain);
...
Action.FightFrostGiants = new MultipartAction('Fight Frost Giants', {
  ...
  segmentFinished() {
    ...
    if (vals.curFightFrostGiantsSegment >= 6) setStoryFlag('giantGuildRankEReached');
    if (vals.curFightFrostGiantsSegment >= 12) setStoryFlag('giantGuildRankDReached');
    if (vals.curFightFrostGiantsSegment >= 18) setStoryFlag('giantGuildRankCReached');
    if (vals.curFightFrostGiantsSegment >= 24) setStoryFlag('giantGuildRankBReached');
    if (vals.curFightFrostGiantsSegment >= 30) setStoryFlag('giantGuildRankAReached');
    if (vals.curFightFrostGiantsSegment >= 36) setStoryFlag('giantGuildRankSReached');
    if (vals.curFightFrostGiantsSegment >= 42) {
      setStoryFlag('giantGuildRankSSReached');
    }
    if (vals.curFightFrostGiantsSegment >= 48) {
      setStoryFlag('giantGuildRankSSSReached');
    }
    if (vals.curFightFrostGiantsSegment >= 54) setStoryFlag('giantGuildRankUReached');
    if (vals.curFightFrostGiantsSegment >= 60) {
      setStoryFlag('giantGuildRankGodlikeReached');
    }
  },
  getPartName() {
...
Action.GreatFeast = new MultipartAction('Great Feast', {
  ...
  canStart(loopCounter = towns[this.townNum].GreatFeastLoopCounter) {
    return resources.reputation >= 100 && loopCounter === 0 && checkSoulstoneSac(this.goldCost()) &&
      getBuffLevel('Feast') < getBuffCap('Feast');
  },
  loopCost(segment) {
...
export function adjustPylons() {
  ...
  town.totalPylons = Math.floor(
    base * getSkillMod('Spatiomancy', 1000, 1200, .5) + base * getSurveyBonus(town),
  );
}
...
Action.DarkSacrifice = new Action('Dark Sacrifice', {
  ...
  storyReqs(storyNum) {
    switch (storyNum) {
      case 1:
        return getSkillLevel('Commune') >= 1;
      case 2:
        return getSkillLevel('Commune') >= 100;
      case 3:
        return getSkillLevel('Commune') >= 1000;
    }
  },
  stats: {
    Int: 0.2,
  ...
  finish() {
    handleSkillExp(this.skills);
    view.requestUpdate('adjustGoldCost', {
      varName: 'DarkRitual',
...
Action.FightJungleMonsters = new MultipartAction('Fight Jungle Monsters', {
  ...
  segmentFinished() {
    ...
    if (vals.curFightJungleMonstersSegment > 8) {
      setStoryFlag('monsterGuildRankDReached');
    }
    if (vals.curFightJungleMonstersSegment > 14) {
      setStoryFlag('monsterGuildRankCReached');
    }
    if (vals.curFightJungleMonstersSegment > 20) {
      setStoryFlag('monsterGuildRankBReached');
    }
    if (vals.curFightJungleMonstersSegment > 26) {
      setStoryFlag('monsterGuildRankAReached');
    }
    if (vals.curFightJungleMonstersSegment > 32) {
      setStoryFlag('monsterGuildRankSReached');
    }
    if (vals.curFightJungleMonstersSegment > 38) {
      setStoryFlag('monsterGuildRankSSReached');
    }
    if (vals.curFightJungleMonstersSegment > 44) {
      setStoryFlag('monsterGuildRankSSSReached');
    }
    if (vals.curFightJungleMonstersSegment > 50) {
      setStoryFlag('monsterGuildRankUReached');
    }
    if (vals.curFightJungleMonstersSegment > 56) {
      setStoryFlag('monsterGuildRankGodlikeReached');
    }
    // Additional thing?
  },
  getPartName() {
...
Action.PrepareBuffet = new Action('Prepare Buffet', {
  ...
  storyReqs(storyNum) {
    switch (storyNum) {
      ...
      case 4:
        return getSkillLevel('Gluttony') >= 10;
      case 5:
        return getSkillLevel('Gluttony') >= 100;
    }
  },
  stats: {
    Con: 0.3,
...
Action.Totem = new Action('Totem', {
  ...
  storyReqs(storyNum) {
    switch (storyNum) {
      case 1:
        return getSkillLevel('Wunderkind') >= 1;
      case 2:
        return getSkillLevel('Wunderkind') >= 5;
      case 3:
        return getSkillLevel('Wunderkind') >= 60;
      case 4:
        return getSkillLevel('Wunderkind') >= 360;
    }
  },
  stats: {
    Con: 0.3,
...
Action.Excursion = new Action('Excursion', {
  ...
  finish() {
    if (vals.guild === 'Thieves' || vals.guild === 'Explorer') {
      setStoryFlag('excursionAsGuildmember');
    }
...
export function adjustPockets() {
  ...
  town.totalPockets = Math.floor(
    base * getSkillMod('Spatiomancy', 1100, 1300, .5) + base * getSurveyBonus(town),
  );
  view.requestUpdate('updateActionTooltips', null);
}
export function adjustWarehouses() {
  ...
  town.totalWarehouses = Math.floor(
    base * getSkillMod('Spatiomancy', 1200, 1400, .5) + base * getSurveyBonus(town),
  );
  view.requestUpdate('updateActionTooltips', null);
}
export function adjustInsurance() {
  ...
  town.totalInsurance = Math.floor(
    base * getSkillMod('Spatiomancy', 1300, 1500, .5) + base * getSurveyBonus(town),
  );
  view.requestUpdate('updateActionTooltips', null);
}
...
Action.ThievesGuild = new MultipartAction('Thieves Guild', {
  ...
  finish() {
    ...
    if (vals.curThievesGuildSegment >= 3) setStoryFlag('thiefGuildRankEReached');
    if (vals.curThievesGuildSegment >= 6) setStoryFlag('thiefGuildRankDReached');
    if (vals.curThievesGuildSegment >= 9) setStoryFlag('thiefGuildRankCReached');
    if (vals.curThievesGuildSegment >= 12) setStoryFlag('thiefGuildRankBReached');
    if (vals.curThievesGuildSegment >= 15) setStoryFlag('thiefGuildRankAReached');
    if (vals.curThievesGuildSegment >= 18) setStoryFlag('thiefGuildRankSReached');
    if (vals.curThievesGuildSegment >= 30) setStoryFlag('thiefGuildRankUReached');
    if (vals.curThievesGuildSegment >= 42) {
      setStoryFlag('thiefGuildRankGodlikeReached');
    }
  },
});
...
Action.ImbueSoul = new MultipartAction('Imbue Soul', {
  ...
  canStart(loopCounter = towns[8].ImbueSoulLoopCounter) {
    return loopCounter === 0 && getBuffLevel('Imbuement') > 499 &&
      getBuffLevel('Imbuement2') > 499 &&
      getBuffLevel('Imbuement3') < 7;
  },
  loopCost(segment) {
    return 100000000 * (segment * 5 + 1);
  },
...
export const actionNameMap = {
  ...
  'AssassinZ7': 'assassin',
  // town 1
  'Map': 'townZ0.options.map',
  'Wander': 'townZ0.options.wander',
  'Smash Pots': 'townZ0.options.smashPots',
  'Pick Locks': 'townZ0.options.pickLocks',
  'Buy Glasses': 'townZ0.options.buyGlasses',
  'Found Glasses': 'townZ0.options.foundGlasses',
  'Buy Mana Z1': 'townZ0.options.buyManaZ1',
  'Meet People': 'townZ0.options.meetPeople',
  'Train Strength': 'townZ0.options.trainStrength',
  'Short Quest': 'townZ0.options.shortQuest',
  'Investigate': 'townZ0.options.investigate',
  'Long Quest': 'townZ0.options.longQuest',
  'Throw Party': 'townZ0.options.throwParty',
  'Warrior Lessons': 'townZ0.options.warriorLessons',
  'Mage Lessons': 'townZ0.options.mageLessons',
  'Heal The Sick': 'townZ0.options.healTheSick',
  'Fight Monsters': 'townZ0.options.fightMonsters',
  'Small Dungeon': 'townZ0.options.smallDungeon',
  'Buy Supplies': 'townZ0.options.buySupplies',
  'Haggle': 'townZ0.options.haggle',
  'Start Journey': 'townZ0.journeys.startJourney',
  'Hitch Ride': 'townZ0.journeys.hitchRide',
  'Open Rift': 'townZ0.journeys.openRift',
  // town 2
  'Explore Forest': 'townZ1.options.exploreForest',
  'Wild Mana': 'townZ1.options.wildMana',
  'Gather Herbs': 'townZ1.options.gatherHerbs',
  'Hunt': 'townZ1.options.hunt',
  'Sit By Waterfall': 'townZ1.options.sitByWaterfall',
  'Old Shortcut': 'townZ1.options.oldShortcut',
  'Talk To Hermit': 'townZ1.options.talkToHermit',
  'Practical Magic': 'townZ1.options.practicalMagic',
  'Learn Alchemy': 'townZ1.options.learnAlchemy',
  'Brew Potions': 'townZ1.options.brewPotions',
  'Train Dexterity': 'townZ1.options.trainDexterity',
  'Train Speed': 'townZ1.options.trainSpeed',
  'Follow Flowers': 'townZ1.options.followFlowers',
  'Bird Watching': 'townZ1.options.birdWatching',
  'Clear Thicket': 'townZ1.options.clearThicket',
  'Talk To Witch': 'townZ1.options.talkToWitch',
  'Dark Magic': 'townZ1.options.darkMagic',
  'Dark Ritual': 'townZ1.options.darkRitual',
  'Continue On': 'townZ1.journeys.continueOn',
  // town 3
  'Explore City': 'townZ2.options.exploreCity',
  'Gamble': 'townZ2.options.gamble',
  'Get Drunk': 'townZ2.options.getDrunk',
  'Buy Mana Z3': 'townZ2.options.buyManaZ3',

  'Sell Potions': 'townZ2.options.sellPotions',
  'Adventure Guild': 'townZ2.options.adventureGuild',
  'Gather Team': 'townZ2.options.gatherTeam',
  'Large Dungeon': 'townZ2.options.largeDungeon',

  'Crafting Guild': 'townZ2.options.craftingGuild',
  'Craft Armor': 'townZ2.options.craftArmor',
  'Apprentice': 'townZ2.options.apprentice',
  'Mason': 'townZ2.options.mason',
  'Architect': 'townZ2.options.architect',
  'Read Books': 'townZ2.options.readBooks',
  'Buy Pickaxe': 'townZ2.options.buyPickaxe',
  'Heroes Trial': 'townZ2.options.heroesTrial',
  'Start Trek': 'townZ2.journeys.startTrek',
  'Underworld': 'townZ2.journeys.underworld',
  // town 4
  'Climb Mountain': 'townZ3.options.climbMountain',
  'Mana Geyser': 'townZ3.options.manaGeyser',
  'Decipher Runes': 'townZ3.options.decipherRunes',
  'Chronomancy': 'townZ3.options.chronomancy',
  'Looping Potion': 'townZ3.options.loopingPotion',
  'Pyromancy': 'townZ3.options.pyromancy',
  'Explore Cavern': 'townZ3.options.exploreCavern',
  'Mine Soulstones': 'townZ3.options.mineSoulstones',
  'Hunt Trolls': 'townZ3.options.huntTrolls',
  'Check Walls': 'townZ3.options.checkWalls',
  'Take Artifacts': 'townZ3.options.takeArtifacts',
  'Imbue Mind': 'townZ3.options.imbueMind',
  'Imbue Body': 'townZ3.options.imbueBody',
  'Face Judgement': 'townZ3.journeys.faceJudgement',
  'Guru': 'townZ3.journeys.guru',
  // town 5
  'Guided Tour': 'townZ4.options.guidedTour',
  'Canvass': 'townZ4.options.canvass',
  'Donate': 'townZ4.options.donate',
  'Accept Donations': 'townZ4.options.acceptDonations',
  'Tidy Up': 'townZ4.options.tidyUp',
  'Buy Mana Z5': 'townZ4.options.buyManaZ5',
  'Sell Artifact': 'townZ4.options.sellArtifact',
  'Gift Artifact': 'townZ4.options.giftArtifact',
  'Mercantilism': 'townZ4.options.mercantilism',
  'Charm School': 'townZ4.options.charmSchool',
  'Oracle': 'townZ4.options.oracle',
  'Enchant Armor': 'townZ4.options.enchantArmor',
  'Wizard College': 'townZ4.options.wizardCollege',
  'Restoration': 'townZ4.options.restoration',
  'Spatiomancy': 'townZ4.options.spatiomancy',
  'Seek Citizenship': 'townZ4.options.seekCitizenship',
  'Build Housing': 'townZ4.options.buildHousing',
  'Collect Taxes': 'townZ4.options.collectTaxes',
  'Pegasus': 'townZ4.options.pegasus',
  'Fight Frost Giants': 'townZ4.options.fightFrostGiants',

  'Seek Blessing': 'townZ4.options.seekBlessing',
  'Great Feast': 'townZ4.options.greatFeast',

  'Fall From Grace': 'townZ4.journeys.fallFromGrace',
  // town 6
  'Meander': 'townZ5.options.meander',
  'Mana Well': 'townZ5.options.manaWell',
  'Destroy Pylons': 'townZ5.options.destroyPylons',
  'Raise Zombie': 'townZ5.options.raiseZombie',
  'Dark Sacrifice': 'townZ5.options.darkSacrifice',
  'The Spire': 'townZ5.options.theSpire',
  'Purchase Supplies': 'townZ5.options.purchaseSupplies',
  'Dead Trial': 'townZ5.options.deadTrial',
  'Journey Forth': 'townZ5.journeys.journeyForth',
  // town 7
  'Explore Jungle': 'townZ6.options.exploreJungle',
  'Fight Jungle Monsters': 'townZ6.options.fightJungleMonsters',
  'Rescue Survivors': 'townZ6.options.rescueSurvivors',
  'Prepare Buffet': 'townZ6.options.prepareBuffet',
  'Totem': 'townZ6.options.totem',
  'Escape': 'townZ6.journeys.escape',
  'Open Portal': 'townZ6.journeys.openPortal',
  // town 8
  'Excursion': 'townZ7.options.excursion',
  'Explorers Guild': 'townZ7.options.explorersGuild',
  'Thieves Guild': 'townZ7.options.thievesGuild',
  'Pick Pockets': 'townZ7.options.pickPockets',
  'Rob Warehouse': 'townZ7.options.robWarehouse',
  'Insurance Fraud': 'townZ7.options.insuranceFraud',
  'Guild Assassin': 'townZ7.options.guildAssassin',
  'Invest': 'townZ7.options.invest',
  'Collect Interest': 'townZ7.options.collectInterest',
  'Seminar': 'townZ7.options.seminar',
  'Purchase Key': 'townZ7.options.purchaseKey',
  'Secret Trial': 'townZ7.options.secretTrial',
  'Leave City': 'townZ7.journeys.leaveCity',
  // town 9
  'Imbue Soul': 'townZ8.options.imbueSoul',
  'Build Tower': 'townZ8.options.buildTower',
  'Gods Trial': 'townZ8.options.godsTrial',
  'Challenge Gods': 'townZ8.options.challengeGods',
  'Restore Time': 'townZ8.options.restoreTime',
};
