...
class ReorderableList extends BaseComponent {
  ...
  set accepts(v) {
    this.setAttribute('accepts', v);
  }

  get acceptsList() {
    return this.accepts?.split(' ').flatMap((s) => s || undefined) ?? [];
  }
...
class ListItem extends BaseComponent {
  ...
  set expandable(v) {
    this.toggleAttribute('expandable', v);
  }
  ...
  set reorderable(v) {
    this.toggleAttribute('reorderable', v);
  }
  ...
  set outside(v) {
    this.toggleAttribute('outside', v);
  }
  ...
  set optional(v) {
    this.toggleAttribute('optional', v);
  }

  get present() {
    return this.hasAttribute('present');
  }
  set present(v) {
    this.toggleAttribute('present', v);
  }
  ...
  set dragging(v) {
    this.toggleAttribute('dragging', v);
  }
  ...
  set label(v) {
    if (this.label !== v) this.setAttribute('label', v);
  }
  ...
  dragStartHandler(event) {
    ...
    globalThis.addEventListener('pointerdown', this.dragStartHandler);
    globalThis.addEventListener('pointermove', this.dragMoveHandler);
    globalThis.addEventListener('pointerup', this.dragEndHandler);
  ...
  endDrag(event = null, revert = false) {
    ...
    globalThis.removeEventListener('pointerdown', this.dragStartHandler);
    globalThis.removeEventListener('pointermove', this.dragMoveHandler);
    globalThis.removeEventListener('pointerup', this.dragEndHandler);
    ...
    if (
      event instanceof PointerEvent && event.type === 'pointermove' &&
      (event.buttons & 2 /* right button */) !== 0 /* is held */
    ) {
  ...
  dragMoveHandler(event) {
    ...
    for (const element of elements) {
      if (element instanceof ListItem) {
        ...
        try {
          if (
            targetList === this.parentElement &&
            (this.compareDocumentPosition(element) & this.DOCUMENT_POSITION_FOLLOWING) !== 0
          ) {
  ...
  dragEndHandler(event) {
    ...
    if (event.type === 'pointerup') {
      ...
      globalThis.addEventListener('click', this.dragEndHandler, { once: true, capture: true });
      // but in case it doesn't come, fire it off ourself and remove the override
      requestAnimationFrame(() => {
        this.endDrag(null, false);
        globalThis.removeEventListener('click', this.dragEndHandler, { capture: true });
      });
      event.preventDefault();
      event.stopPropagation();
      return;
