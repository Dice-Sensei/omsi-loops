...
class GoogleCloud {
  ...
  async init() {
    if (!globalThis.google || !globalThis.gapi) {
      ...
      while (!globalThis.google || !globalThis.gapi) {
        await delay(100);
        if (timeoutCounter-- === 0) {
  ...
  async authorize(userRequest) {
    console.log('starting authorize for', {
      userRequest,
      promise: this.#authzPromise,
      ...
      () =>
        new Promise((resolve, reject) => {
          if (userRequest && (gapi.client.getToken() == null || Date.now() > this.tokenExpiry)) {
            ...
            this.#tokenHandler = (success) =>
              success ? resolve(this.tokenResponse?.access_token) : reject(this.tokenError?.message);
